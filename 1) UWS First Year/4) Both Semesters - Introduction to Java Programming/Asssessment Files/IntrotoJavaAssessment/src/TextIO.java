...
/**
 * TextIO provides a set of static methods for reading and writing text. This
 * version of TextIO is an alternative to the basic version (which uses standard
 * input and standard output for its basic IO operations). When this version is
 * used, a window appears on the screen. This window simulates standard IO when
 * it is used by TextIO: When TextIO writes to "standard output", the output
 * actually goes to the window. When TextIO reads from "standard input", the
 * input actually comes from input typed into the window by the user. This is
 * the default behaviour, but it is possible to redirect the IO to files or to
 * other input and output streams. When the (simulated) standard input and
 * output streams are being used, the input methods will not produce an error;
 * instead, the user is repeatedly prompted for input until a legal input is
 * entered. For the most part, any other error will be translated into an
 * IllegalArguementException. Note that when this version of TextIO is used,
 * calls to the output methods in TextIO are not equivalent to calls to methods
 * in System.out; output to System.out still goes to the usual standard output.
 * 
 * The window in which IO takes place remains open until it is closed by the
 * user, even after the program that used TextIO ends.
 * 
 ...
 * single source file, all the methods are static methods, and none of the
 * methods throw exceptions that would require try...catch statements. Also for
 * this reason, all exceptions are converted into IllegalArgumentExceptions,
 * even when this exception type doesn't really make sense.
 * 
 * When this version of TextIO is compiled, a rather large number of .class
 * files are produced. The class files all have names beginning with "TextIO".
 * All of the class files are required for TextIO to run successfully.
 * 
...
public class TextIO {
	 ...
	 * the previous input source to be read after the new source has been selected.
	 ...
	 * provided it is non-null. If inputStream is null, then this method has 
	 * the same effect as calling readStandardInput(); that is, future input 
	 * will come from the standard input stream.
	 */
	...
	/**
	 * After this method is called, output will be written to standard
	 * output (as it is in the default state). If a file or stream was
	 * previously open for output, it will be closed.
	 */
	 ...
	 * provided it is non-null. If outputStream is null, then this method 
	 * has the same effect as calling writeStandardOutput(); that is,  
	 * future output will be sent to the standard output stream.
	 */
	 ...
	 * standard output. If an error occurs, the output destination is not changed.
	 * 
	 * NOTE: Calling this method starts a GUI user interface thread, which can
	 * continue to run even if the thread that runs the main program ends. If
	 * you use this method in a non-GUI program, it might be necessary to call
	 ...
	 * If you want to do that, call skipBlanks() first.
	 */
	 ...
	 * testing for end-of-line.
	 * If you want to do that, call skipBlanks() or skipWhitespace() first.
	 */
	 ...
	 * end-of-line in the input source is something else, such as '\r' or "\r\n".
	 * This method will throw an IllegalArgumentException if the input is at 
	 * end-of-file (which will not ordinarily happen if reading from standard input).
	 */
	 ...
	 * end-of-file, or a non-whitespace character. This method never causes an error. 
	 * (Ordinarily, end-of-file is not possible when reading from standard input.)
	 */
	...
	// Wraps the input stream from Console. (Set in static initialiser.)
	private final static BufferedReader standardInput;
	
	// Wraps the output stream. (Set in static initialiser.)
	private final static PrintWriter standardOutput;

	// Stream that data is read from; the current input source. (Set in static initialiser.)
	private static BufferedReader in;
	
	// Stream that data is written to; the current output destination. (Set in static initialiser.)
	...
	// The GUI IO widget used for simulated input/output.
	 ...
	 * Defining the GUI.
	 * 
	 * A "Console" is a panel which simulates standard input/output. When this GUI  
	 * TextIO class is used, a frame will be opened that displays a Console.
	 * When the output destination for TextIO is standard output, the output will
	 * appear in the window. When TextIO wants to fill its buffer with a line of
	 * input from standard input, the user will type a line of input in the Console,
	 * which will go into the buffer. There is also a scroll bar that can be used to
	 * browse lines that have scrolled off the top (up to 2000 lines).
	 */
	...
	private static class Console extends JPanel {
		...
		Console() {
			...
			setFont(f);
			setPreferredSize(new Dimension(2 * MARGIN + 80 * charWidth,
					2 * MARGIN + (25 - 1) * lineSkip + fontMetrics.getAscent() + fontMetrics.getDescent()));
		...
		public void paintComponent(Graphics g) {
			...
			if (doingInput && cursorOn) {
				...
				int y1 = MARGIN + (lineCount - 1 - topLine) * lineSkip + fontMetrics.getAscent()
		...
		// "Standard input" is replaced in TextIO with an object of this type.
		...
		// "Standard output" is replaced in TextIO with an object of this type.
	...
	// Static initialiser opens the GUI TextIO window.
	static { 
