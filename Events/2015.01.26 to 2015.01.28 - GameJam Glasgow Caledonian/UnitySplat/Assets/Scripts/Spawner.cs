...
public class Spawner : MonoBehaviour {
    ...
    void Start()
    {
        ...
        PlayerBody = Game.PlayerBody.GetComponent<Rigidbody2D>();
    }
    ...
    public void Spawn(GameObject objToSpawn)
    {
        ...
        newObj.GetComponent<Rigidbody2D>().velocity = PlayerBody.velocity;

        if (newObj.CompareTag("Enemy"))
        {
            newObj.GetComponent<Rigidbody2D>().AddForce(new Vector2(0, -600f));
        }
        newObj.GetComponent<Rigidbody2D>().AddForce(Force);
